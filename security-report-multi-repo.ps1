...
try {
    ...
    Write-Host "ERROR: ImportExcel module not found. Please install it:" -ForegroundColor Red
    Write-Host "  Install-Module ImportExcel -Force" -ForegroundColor Yellow
    exit
}
...
$excelPath = "$OutputPath\Security-Report-$timestamp.xlsx"
...
# Create Excel file with separate sheets for each repo
foreach ($repo in $allRepoData.Keys) {
    $sheetName = $repo.Replace("/", "-")
...
Write-Host ""
Write-Host "Report location: $excelPath" -ForegroundColor Green
Write-Host ""
